<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="/ghi-chu" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300 shadow-lg">
                + Thêm ghi chú
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <form action="/danh-sach" method="GET" class="flex flex-wrap gap-2 w-full md:w-auto">
                    ...
                    @if(request('search') || request('category_id'))
                        <a href="/danh-sach" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 flex items-center shadow-sm">
                            Xóa lọc
                        </a>
                    @endif
                </form>

                <a href="/thung-rac" class="text-sm text-gray-500 hover:text-red-500 flex items-center gap-1 transition-colors group">
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($notes as $item)
                        ...
                        <div class="{{ $currentColor }} overflow-hidden shadow-sm sm:rounded-lg border-2 {{ $item->is_pinned ? 'border-blue-400' : 'border-gray-200' }} hover:shadow-md transition-all duration-300 relative">
                            @if($item->is_pinned)
                            ...
                            <div class="p-6">
                                ...
                                <p class="text-gray-800 text-lg mb-6 leading-relaxed min-h-[100px]">
                                    {{ $item->content }}
                                </p>
                                ...
                                <div class="flex justify-between items-center border-t border-gray-200 pt-4">
                                    ...
                                    <div class="flex space-x-3">
                                        <a href="/ghim-ghi-chu/{{ $item->id }}" class="{{ $item->is_pinned ? 'text-blue-600' : 'text-gray-400' }} hover:text-blue-800 font-semibold text-sm">
                                            {{ $item->is_pinned ? 'Bỏ ghim' : 'Ghim' }}
                                        </a>
                                        <a href="/sua-ghi-chu/{{ $item->id }}" class="text-amber-600 hover:text-amber-800 font-semibold text-sm">Sửa</a>
                                        <a href="/xoa-ghi-chu/{{ $item->id }}" onclick="return confirm('Xóa nhé?')" class="text-red-600 hover:text-red-800 font-semibold text-sm">Xóa</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {{-- PHẦN HIỂN THỊ PHÂN TRANG NẰM Ở ĐÂY --}}
                <div class="mt-8">
